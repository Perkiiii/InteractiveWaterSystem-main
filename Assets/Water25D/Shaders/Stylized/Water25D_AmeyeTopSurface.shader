Shader "Water25D/Stylized Ameye Top Surface"
{
    Properties
    {
        ...
        half4 Frag(Varyings input) : SV_Target
        {
            ...
            if (_StylizedHighlightsEnabled > 0.5)
            {
                ...
                float ameyeHighlight = Water25DAmeyeSpecular(
                    ...
                    _HighlightSoftness,
                    _HighlightBreakup);
                highlight = max(highlight, ameyeHighlight * saturate(1.0 - _HighlightThreshold));
            ...
            if (_RefractionEnabled > 0.5 && _RefractionSourceAvailable > 0.5)
            {
                ...
                float2 sourceRefractionUV;
                DistortUV_float(
                    screenUV,
                    saturate(_RefractionStrength * 100.0),
                    sourceRefractionUV);
                screenUV = lerp(
                    screenUV + normalWS.xz * _RefractionStrength,
                    sourceRefractionUV,
                    saturate(_RefractionStrength * 40.0));
