...
The connected repository’s latest `main` revision remains commit `200c840b60c7ab139bc7e4743a2b7a2082227504`, committed on 1 August 2026. The implementation state therefore matches the completed Pass 1 audit: the package contains a working per-water Custom Render Texture ripple backend, generated XZ and XY meshes, shared reflection grouping, pooled particle effects, profiles, editor tooling, tests and benchmark scaffolding, but it still lacks measured target-device profiling, clean-project validation, comprehensive interaction tests and production-grade flat-stylised rendering. The architectural decisions in Pass 1 should be treated as fixed inputs rather than reconsidered. fileciteturn0file0
...
The current repository documentation records 24 passing EditMode tests and one passing PlayMode test, but also explicitly identifies clean-project import, allocation profiling, target-device benchmarking and full planar-reflection validation as incomplete. Those are release blockers rather than optional follow-up work. fileciteturn16file0
...
The interface is deliberately narrow: availability, suspension state, output height texture, dropped-impact diagnostics, impact enqueueing, ticking, reset and disposal. Gameplay does not sample the GPU state, which is an important architectural property because it prevents visual resolution or GPU readback from becoming gameplay-authoritative. fileciteturn5file0

`WaterRippleModule` owns exactly one simulator for a water object. It recreates that simulator when the water dimensions, simulation-relevant quality values or material template change, and disposes it through the instance-owned runtime resource container. fileciteturn6file0
...
These values come directly from `WaterQualitySettings` and the resource creation path. fileciteturn7file0 fileciteturn8file0
...
At 30 Hz with two full propagation substeps, the default water processes 1,996,800 propagated pixel executions per second while active. The propagation shader samples the current and previous height plus four neighbours, so its shader work is materially greater than that pixel count alone suggests. Each queued impact also performs a bounded update pass. fileciteturn21file0

### Correctness problems that must be fixed

The most serious backend issue is deferred Custom Render Texture update state. The simulator currently changes `_ImpactHeight`, `_ImpactCenter`, `_ImpactRadius` and the update-zone array, calls `CustomRenderTexture.Update(1)`, then repeats that process for the next impact. fileciteturn4file0

Unity documents that `Update()` does not execute immediately. Requested updates occur at the beginning of a subsequent frame using the then-current Custom Render Texture state. Unity specifically warns that changing an update-zone array between two `Update()` calls can cause both updates to use the second array. Double buffering also performs a texture copy on each swap, with cost dependent on resolution and update frequency. citeturn0search10turn0search11turn0search13

The existing test verifies only that four logical impact-update requests and one propagation request were issued. It does not verify that four spatially distinct impacts appear in four distinct texture regions. It therefore cannot detect the deferred-state defect. fileciteturn17file0

The production fix should be **one immutable impact batch per simulation step**, rather than one mutable-material request per impact. The preferred implementation is:
...
The second defect is idle suspension. When no impacts are pending, `_idleTime` advances only while the renderer is invisible. If the renderer is visible, idle time is reset to zero every tick, so a visible calm water never suspends. fileciteturn4file0
...
`Water25DController.TryGetSurfaceUV` transforms the requested world position into the water root’s local space and maps local X/Z to `[0,1]²`, rejecting points outside the rectangular top surface. `GetInteractionWorldPosition` converts 2D gameplay X/Y to the configured top-surface depth lane. This mapping is suitable for both CRT impacts and procedural rings, although it should be centralised into the Pass 1 `WaterSurfaceMapping` value type. fileciteturn15file0

The CRT currently affects only the top shader. Its red channel is sampled in the vertex shader and multiplied by `_RippleAmplitude`; the resulting value is added to vertex Y together with analytical ambient waves. The front shader does not sample the CRT. It uses only analytical ambient displacement, multiplied by front-mesh UV Y so that the upper front edge moves with the top. fileciteturn12file0 fileciteturn13file0
...
> **Disable CRT allocation and execution entirely in `FlatStylized`. Retain it temporarily as an explicitly optional `SimulatedRipples` compatibility mode. Do not combine it with procedural rings by default. Add optional CRT-derived normal and reflection distortion only inside `SimulatedRipples`, after the batching defect is fixed and profiled.**
...
Required implementation changes are:
...
The current top shader uses transparent alpha blending, `ZWrite Off` and `Cull Off`, and it supports both `Universal2D` and `UniversalForward` passes. It currently performs ambient and CRT displacement in the vertex stage, adds edge foam in the fragment stage, and either uses a simple stylised gradient or projects into a planar reflection texture. fileciteturn12file0

The front shader also uses transparent blending, `ZWrite Off` and `Cull Off`. It vertically displaces vertices using ambient waves and renders a shallow-to-deep colour gradient with a narrow top foam band. fileciteturn13file0
...
The current `WaterReflectionModule` disposes its old registration every time `Configure` is called, then creates a new registration if Play Mode is active and reflection is not disabled. fileciteturn9file0
...
Unity’s SRP render-request API supports URP single-camera requests and provides a pipeline-native way to render a camera outside the ordinary loop. The current `Camera.Render()` path should not be replaced speculatively, but both paths must be compared under the project’s Unity 6 and URP version. citeturn3search5turn3search7turn3search14
...
At present, `WaterRenderingModule` clears and rewrites one property block, resetting reflection values to identity and disabled, while `WaterReflectionManager` later reads and modifies the block. fileciteturn14file0 fileciteturn10file0
...
The core flat shader must not require `_CameraDepthTexture` or `_CameraOpaqueTexture`. URP creates those textures only when the corresponding pipeline or camera setting is enabled, and 2D Renderer projects may not use all conventional 3D URP settings in the same way. citeturn3search11
...
The official Unity Asset Store listing identifies Toon Water URP by Piotr T. as an Extension Asset. The latest listed version is **1.19**, released on **17 July 2022**, with an original Unity version of **2019.3.0**. The official page does not declare Unity 6, URP 17 or Render Graph compatibility. citeturn1view0
...
These descriptions also state that planar reflections are desktop-only and that the custom Inspector was designed around Unity 2019.4.5 and URP 7.4.3 or later. citeturn2search2turn2search4turn2search8
...
The Asset Store page classifies Toon Water URP as an Extension Asset. Unity’s current EULA grants Extension Assets on a per-seat basis and restricts redistribution or commercialisation of the raw asset except as incorporated into a qualifying finished product. It does not grant permission to include the package’s source Shader Graphs, scripts or textures as redistributable Water25D package content. citeturn1view0turn4search0
...
must remain source-compatible. fileciteturn15file0
...
| `Assets/Water25D/Runtime/Physics/WaterContactTracker.cs` | New reusable fixed-capacity body tracker. | None. |
...
The existing benchmark driver can create up to 32 waters and 64 dynamic bodies and issue deterministic impacts, but it currently records no profiler measurements and primarily targets CRT impacts. fileciteturn20file0

The existing PlayMode test proves that a default controller creates a 320 × 104 CRT, accepts an impact and recreates the texture at 160 × 104 after resizing. It does not test flat mode, cleanup, interaction qualification, reflection recursion, pooling allocations or rendering output. fileciteturn18file0
...
The package-dependency test should scan every package asset rather than searching source text alone:
...
The current surface interaction component already deduplicates colliders by attached `Rigidbody2D`, but it emits an entry on the first trigger-enter callback and an exit on the last trigger-exit callback using the collider bounds centre. It does not prove that the body crossed the top waterline; side entry into the thin trigger can therefore create a ripple and splash. fileciteturn19file0
...
Unity’s 2D Renderer sorting order depends on sorting layer/order, render queue, camera distance and sorting groups. Orthographic and perspective modes use different distance interpretations, so both must be captured. citeturn3search2
...
The repository currently states that package code has no runtime, editor or serialized dependency on the identified reference systems, but a clean-project execution has not yet been completed. fileciteturn16file0

## Final production decisions

| Required decision | Direct answer |
|---|---|
| Final role of the CRT backend | Retain temporarily as optional `SimulatedRipples`. Allocate and tick it only in that mode. Fix impact batching and suspension before production support. Permit optional normal/reflection influence in simulated mode, but no CRT use in `FlatStylized`. Begin deprecation only after adoption and profiling evidence. |
...
| Exact files requiring modification | Controller, geometry, rendering, reflection, ripple, physics interaction, profiles, FX, three shaders, material defaults, editor defaults/menu/Inspector/validation, benchmark, tests and documentation as listed in the migration table. Add the presentation module, render-data type, mode enum, contact tracker, flat profiles and migration utility. |
| Automated and manual validation | Add serialization, migration, geometry, interaction, pooling, CRT spatial correctness, reflection lifecycle, cleanup, optional-dependency and package-dependency tests. Capture flat silhouette, interactions, seam, reflection modes, orthographic projection and sprite sorting. |
...
| Criteria for making FlatStylized the default | Flat mode is already the default for newly created objects once it passes no-displacement, interaction placement, cleanup, reflection, visual-comparison and clean-project tests and proves materially cheaper than simulated mode. Existing objects are not automatically switched. |
